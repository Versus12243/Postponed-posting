    ...
    public class SocialNetworksService: ISocialNetworksService
    {
        ...
        public List<AvailableSocialNetworkViewModel> GetAviableSNForUser(string UserId)
        {
            ...
            foreach(var network in userSN)
            {
                ...
                sn.CredentialsIsActive = sn.CredentialsExist && network.Credentials.EntityStatus == Domain.Entities.StatusEnums.EntityStatus.Active;
                sn.Name = network.SocialNetwork.Name;
                sn.IsActive = network.Credentials.Status == Domain.Entities.StatusEnums.CredentialsStatus.Active && network.Status == Domain.Entities.StatusEnums.EntityStatus.Active;
